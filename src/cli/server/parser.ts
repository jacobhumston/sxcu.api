 ...
 */
export type ParsedDataText = { type: 'text'; value: string };
 ...
 */
export type ParsedData = { [key: string]: ParsedDataText | ParsedDataFile };
 ...
 */
export default function parse(data: Buffer): ParsedData {
    const parsedData: ParsedData = {};

    const lines: { string: string; buffer: Buffer }[] = [];
    ...
    const sections: { lines: { string: string; buffer: Buffer }[] }[] = [];
    const header: string = cleanString(lines[0].string);
